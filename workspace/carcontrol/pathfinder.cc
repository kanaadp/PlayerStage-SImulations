...
void onCrash(SimulationProxy *playerSim, int robotnum) {
	...
	cout << scenario << " Robot " << robotnum << ": " << cname
...
void victory(SimulationProxy *playerSim, int robotnum) {
	...
	playerSim->SetPose2d((char*) cname.c_str(), 20, 22, 0);
	cout << scenario << " Robot " << robotnum << ": " << " Congratulations! "
			<< cname << " has run the gauntlet! It is now being removed."
			<< "\n";
}
...
//Begin Control code
	...
	try {
		...
		double final = endtime - initime;
		cout << scenario <<" Robot " << robotprops::properties.Robotnum << ":  Finished in = " << final;
