...
void onCrash(SimulationProxy *playerSim, int robotnum) {
	...
	cout << "Robot " << robotprops::properties.Robotnum << ": " << "Robot "
			<< robotnum << ": " << cname << " has been removed and disabled"
			<< "\n";
}
void victory(SimulationProxy *playerSim, int robotnum) {
	...
	std::string cname = ss.str();
	cout << "Robot " << robotprops::properties.Robotnum << ": " << "Robot "
			<< robotnum << ": " << " Congratulations! " << cname
...
double timeUntil(intent* props, double targetx, double targety,
	...
	if (fabs(robotx) > 3 || fabs(roboty) > 3) {
		...
		cout << "Robot " << robotprops::properties.Robotnum << ": "
				<< "Distance to intersection = " << distancetointersection
				<< "\n";
		cout << "Robot " << robotprops::properties.Robotnum << ": "
				<< "Time to intersection = " << timetointersection << "\n";
	}
	...
	cout << "Robot " << robotprops::properties.Robotnum << ": "
			<< props->robotyaw << "<--- Robotyaw \n";
	cout << "Robot " << robotprops::properties.Robotnum << ": " << idealyaw
			<< "<--- Ideal yaw \n";
	if (fabs(props->robotyaw - idealyaw) > .75) {
		...
		cout << "Robot " << robotprops::properties.Robotnum << ": "
				<< "Distance =" << distance << "\n";
		cout << "Robot " << robotprops::properties.Robotnum << ": " << "Time ="
				<< time << "\n";
	}
	time = time + timetointersection;
	cout << "Robot " << robotprops::properties.Robotnum << ": "
			<< "ETA to collision = " << time << "\n";
	return time;
...
bool process(intent *received) {
	...
	if (collides(collisionx, collisiony, received->initLane, received->turn)) {
		...
		cout << "Robot " << robotprops::properties.Robotnum << ": " << "coly = "
				<< collisiony << "\n";
		cout << "Robot " << robotprops::properties.Robotnum << ": " << "colx = "
				<< collisionx << "\n";
		...
		if (fabs(etacollision - myetacollision) < 1.5) {
			cout << "Robot " << robotprops::properties.Robotnum << ": "
					<< "Collision Predicted!" << "\n";
			double incspeed = (mydistance)
					/ (etacollision - 2.25 - mytimetilintersection);
			double safespeed;
			cout << "Robot " << robotprops::properties.Robotnum << ": "
					<< "A safe increased speed is = " << incspeed << "\n";
...
void recieve(playerc_blackboard_t *x, player_blackboard_entry_t output) {
	...
	time(&rawtime);
	cout << "Robot " << robotprops::properties.Robotnum << ": "
			<< "Current Time = " << rawtime << "\n";
	cout << "Robot " << robotprops::properties.Robotnum << ": "
			<< "Blackboard group: " << output.group << " updated @ "
			<< output.timestamp_sec << "\n";
	cout << "Robot " << robotprops::properties.Robotnum << ": " << "Key = "
			<< output.key << "\n";
	intent recievedprocess;
	int datacount = output.data_count;
	cout << "Robot " << robotprops::properties.Robotnum << ": " << "datacount ="
			<< datacount << "\n";
